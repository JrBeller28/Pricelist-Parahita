...
export default function App() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const fetchPromises = urls.map(async (info) => {
        const rows = await fetchGoogleSheetCsv(info.url);
        return { cat: info.cat, rows };
      });
      ...
      // Process and normalize rows sequentially to maintain proper ID incrementing
      for (const result of results) {
         const parsedRows = result.rows.map(r => normalizeItem(r, result.cat, idCounter++));
  ...
  // Reduced items per page for better DOM performance
    ...
    <div className="min-h-screen flex flex-col bg-surface text-on-background font-body antialiased">
      ...
      <div className={`fixed inset-0 z-[100] lg:hidden transition ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute left-0 top-0 h-full w-72 bg-white shadow-xl transform transition-transform duration-300
          ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
          ...
          <div className="flex items-center justify-between p-4 border-b">
           ...
           {/* Isi Sidebar Mobile */}
          <div className="flex-1 overflow-y-auto p-6 bg-white/90 backdrop-blur-md shadow-[0_10px_40px_rgba(37,99,235,0.08)]">
            ...
            <div className="space-y-1 mb-8">
               {['ALL', 'ACC', 'WOVEN', 'KNITT', 'ACC KNITT', 'FOB'].map(cat => (
                 <button 
                   ...
                   className={`w-full text-left font-body text-sm py-3 px-4 rounded-lg transition-colors ${activeTab === cat ? 'bg-gradient-to-r from-blue-600 to-indigo-500 text-white font-semibold shadow-md' : 'text-on-surface hover:bg-blue-50 hover:text-blue-700 font-medium'}`}>
            ...
            <button onClick={() => {setIsCatalogModalOpen(true); setIsMobileMenuOpen(false);}} className="w-full text-center mt-2 font-body text-xs font-semibold text-primary hover:text-blue-700 bg-blue-50 hover:bg-blue-100 transition-colors py-2 rounded-lg shadow-sm border border-blue-100 mb-2">
              <span className="material-symbols-outlined text-[14px] align-middle mr-1">menu_book</span>
              Lihat Katalog Warna
            </button>
            <button onClick={() => {setIsZipperModalOpen(true); setIsMobileMenuOpen(false);}} className="w-full text-center font-body text-xs font-semibold text-purple-700 hover:text-purple-800 bg-purple-50 hover:bg-purple-100 transition-colors py-2 rounded-lg shadow-sm border border-purple-100">
      ...
      {/* Mobile Bottom Navigation (Katalog vs Analysis) */}
      ...
      {isCatalogModalOpen && (
        ...
        >
           ...
           >
              ...
              <div className="flex-1 overflow-auto bg-surface-container-highest p-4 flex justify-center items-start custom-scrollbar">
                <Document
                  file="/katalog-warna.pdf" 
                  onLoadSuccess={onDocumentLoadSuccess}
                  loading={<span className="animate-pulse font-medium text-on-surface-variant">Memuat Dokumen...</span>}
              ...
              {numPages && (
                <div className="bg-white p-4 border-t border-outline-variant/30 flex justify-center items-center gap-4 z-10 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
                  <button 
                    disabled={pdfPageNumber <= 1} 
                    onClick={() => setPdfPageNumber(p => p - 1)} 
                    className="px-4 py-2 bg-surface-container rounded-lg disabled:opacity-40 font-semibold text-sm hover:bg-surface-variant transition-colors flex items-center gap-1"
                  >
                    <span className="material-symbols-outlined text-[18px]">chevron_left</span> Prev
                  </button>
                  <span className="font-medium text-sm font-body text-on-surface-variant">Halaman <strong className="text-on-surface">{pdfPageNumber}</strong> dari {numPages}</span>
                  <button 
              ...
              {zipperNumPages && (
                <div className="bg-white p-4 border-t border-outline-variant/30 flex justify-center items-center gap-4 z-10 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
                  <button 
                    ...
                    onClick={() => setZipperPdfPageNumber(p => p - 1)} 
                    className="px-4 py-2 bg-purple-50 text-purple-700 rounded-lg disabled:opacity-40 disabled:bg-surface-container font-semibold text-sm hover:bg-purple-100 transition-colors flex items-center gap-1"
                  >
                    <span className="material-symbols-outlined text-[18px]">chevron_left</span> Prev
                  </button>
                  <span className="font-medium text-sm font-body text-on-surface-variant">Halaman <strong className="text-on-surface">{zipperPdfPageNumber}</strong> dari {zipperNumPages}</span>
                  <button 
                    disabled={zipperPdfPageNumber >= zipperNumPages} 
                    onClick={() => setZipperPdfPageNumber(p => p + 1)} 
                    className="px-4 py-2 bg-purple-50 text-purple-700 rounded-lg disabled:opacity-40 disabled:bg-surface-container font-semibold text-sm hover:bg-purple-100 transition-colors flex items-center gap-1"
                  >
                    Next <span className="material-symbols-outlined text-[18px]">chevron_right</span>
