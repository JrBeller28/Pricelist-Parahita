    ...
    <div className="min-h-screen flex flex-col bg-surface text-on-background font-body antialiased">
      ...
      <div className={`fixed inset-0 z-[100] lg:hidden transition ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute left-0 top-0 h-full w-72 bg-white shadow-xl transform transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          {/* Isi Sidebar */}
          <div className="bg-white/90 backdrop-blur-md rounded-2xl p-6 h-[calc(100vh-64px)] overflow-y-auto">
                <div className="flex items-center gap-2 mb-6">
                   <span className="material-symbols-outlined text-primary">list</span>
                   <h2 className="font-headline text-sm text-on-surface font-semibold tracking-widest uppercase">Pricelist Bahan Baku</h2>
                </div>
                <div className="space-y-1 mb-8">
                   {['ALL', 'ACC', 'WOVEN', 'KNITT', 'ACC KNITT', 'FOB'].map(cat => (
                     <button 
                       key={cat} 
                       onClick={() => { setTabAndReset(cat); setIsMobileMenuOpen(false); }}
                       className={`w-full text-left font-body text-sm py-3 px-4 rounded-lg transition-colors ${activeTab === cat ? 'bg-gradient-to-r from-blue-600 to-indigo-500 text-white font-semibold shadow-md' : 'text-on-surface hover:bg-blue-50 font-medium'}`}>
                ...
                <div className="space-y-1 max-h-64 overflow-y-auto pr-2 no-scrollbar">
                   {filteredSuppliers.map((s) => (
                      <button 
                        key={s}
                        onClick={() => { setSupplierFilter(s); setCurrentPage(1); setIsMobileMenuOpen(false); }}
                        className={`w-full text-left font-body text-sm py-2 px-3 rounded-md transition-colors ${supplierFilter === s ? 'bg-blue-50 text-blue-700 border-l-2 border-blue-600 font-medium' : 'text-on-surface-variant hover:text-on-surface hover:bg-blue-50'}`}
                      >
                ...
                <button onClick={() => { setIsCatalogModalOpen(true); setIsMobileMenuOpen(false); }} className="w-full text-center mt-2 font-body text-xs font-semibold text-primary hover:text-blue-700 bg-blue-50 py-2 rounded-lg shadow-sm border border-blue-100 mb-2">
                  <span className="material-symbols-outlined text-[14px] align-middle mr-1">menu_book</span>
                  Lihat Katalog Warna
                </button>
                <button onClick={() => { setIsZipperModalOpen(true); setIsMobileMenuOpen(false); }} className="w-full text-center font-body text-xs font-semibold text-purple-700 hover:text-purple-800 bg-purple-50 py-2 rounded-lg shadow-sm border border-purple-100">
      ...
      <main className="flex-grow pt-20 lg:pt-28 pb-24 lg:pb-16 px-4 lg:px-8 max-w-screen-2xl mx-auto w-full flex flex-col lg:flex-row gap-6 lg:gap-8 bg-[linear-gradient(135deg,#f8fafc_0%,#eef2ff_50%,#f1f5f9_100%)]">
        ...
        <aside className="hidden lg:flex w-72 flex-shrink-0 flex-col gap-6">
          <div className="bg-white/90 backdrop-blur-md rounded-2xl p-6 shadow-[0_10px_40px_rgba(37,99,235,0.08)] border border-white/60">
            ...
            <h3 className="font-label text-xs text-on-surface-variant font-semibold uppercase tracking-widest mt-8 mb-4">Katalog</h3>
            <button onClick={() => setIsCatalogModalOpen(true)} className="w-full text-center mt-2 font-body text-xs font-semibold text-primary hover:text-blue-700 bg-blue-50 py-2 rounded-lg shadow-sm border border-blue-100 mb-2">
              <span className="material-symbols-outlined text-[14px] align-middle mr-1">menu_book</span> Lihat Katalog Warna
            </button>
            <button onClick={() => setIsZipperModalOpen(true)} className="w-full text-center font-body text-xs font-semibold text-purple-700 hover:text-purple-800 bg-purple-50 py-2 rounded-lg shadow-sm border border-purple-100">
              <span className="material-symbols-outlined text-[14px] align-middle mr-1">style</span> Lihat Katalog Aksesories
            </button>
          </div>
        </aside>
      ...
      {selectedProductDetails && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl relative animate-in zoom-in-95">
            <button 
              onClick={() => setSelectedProductDetails(null)} 
              className="absolute top-4 right-4 text-on-surface-variant hover:text-primary rounded-full p-1 bg-surface hover:bg-surface-variant transition-colors"
            >
              <span className="material-symbols-outlined">close</span>
            </button>
            <h3 className="text-xl font-bold mb-4 font-headline text-primary border-b pb-2">Detail Produk</h3>
            <div className="space-y-3 font-body text-sm">
              <p><span className="font-semibold text-on-surface-variant">Nama Barang:</span> {selectedProductDetails.namaBarang}</p>
              <p><span className="font-semibold text-on-surface-variant">Kategori:</span> {selectedProductDetails.kategori}</p>
              <p><span className="font-semibold text-on-surface-variant">Supplier:</span> {selectedProductDetails.supplier}</p>
              <p><span className="font-semibold text-on-surface-variant">Harga Partai:</span> {formatMoney(selectedProductDetails.hargaPartai)} {selectedProductDetails.hargaPartaiSatuan}</p>
              <p><span className="font-semibold text-on-surface-variant">Harga Ecer:</span> {selectedProductDetails.hargaEcer ? `${formatMoney(selectedProductDetails.hargaEcer)} ${selectedProductDetails.hargaEcerSatuan}` : '-'}</p>
              <p><span className="font-semibold text-on-surface-variant">Isi Kemasan:</span> {selectedProductDetails.isiKemasan || '-'}</p>
              <p><span className="font-semibold text-on-surface-variant">Keterangan:</span> {selectedProductDetails.keterangan || '-'}</p>
            </div>
            <div className="mt-6 pt-4 border-t border-outline-variant flex justify-end">
              <button onClick={() => setSelectedProductDetails(null)} className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 font-medium transition-colors">Tutup</button>
            </div>
          </div>
        </div>
      )}

      {/* 2. Modal Katalog Warna (PDF) */}
      {isCatalogModalOpen && (
        <div className="fixed inset-0 z-[200] flex flex-col bg-black/80 p-4 sm:p-8 backdrop-blur-md">
          <div className="flex justify-between items-center mb-4 text-white">
             <h3 className="text-xl font-bold font-headline">Katalog Warna PDF</h3>
             <button onClick={() => setIsCatalogModalOpen(false)} className="bg-white/20 hover:bg-white/30 rounded-full p-2 transition-colors">
               <span className="material-symbols-outlined">close</span>
             </button>
          </div>
          <div className="flex-1 overflow-auto flex justify-center items-start rounded-xl">
             {/* Ganti URL PDF sesuai kebutuhan project */}
             <Document file="/path-to-katalog-warna.pdf" onLoadSuccess={onDocumentLoadSuccess} loading={<div className="text-white">Memuat PDF...</div>}>
                <Page pageNumber={pdfPageNumber} renderAnnotationLayer={false} renderTextLayer={false} className="shadow-2xl" />
             </Document>
          </div>
          {numPages && (
            <div className="mt-4 flex justify-center items-center gap-4 text-white">
              <button disabled={pdfPageNumber <= 1} onClick={() => setPdfPageNumber(p => p - 1)} className="px-3 py-1 bg-white/20 rounded disabled:opacity-50">Prev</button>
              <span>Halaman {pdfPageNumber} dari {numPages}</span>
              <button disabled={pdfPageNumber >= numPages} onClick={() => setPdfPageNumber(p => p + 1)} className="px-3 py-1 bg-white/20 rounded disabled:opacity-50">Next</button>
            </div>
          )}
        </div>
      )}

      {/* 3. Modal Katalog Aksesories (PDF) */}
      {isZipperModalOpen && (
        <div className="fixed inset-0 z-[200] flex flex-col bg-black/80 p-4 sm:p-8 backdrop-blur-md">
          <div className="flex justify-between items-center mb-4 text-white">
             <h3 className="text-xl font-bold font-headline">Katalog Aksesories PDF</h3>
             <button onClick={() => setIsZipperModalOpen(false)} className="bg-white/20 hover:bg-white/30 rounded-full p-2 transition-colors">
               <span className="material-symbols-outlined">close</span>
             </button>
          </div>
          <div className="flex-1 overflow-auto flex justify-center items-start rounded-xl">
             {/* Ganti URL PDF sesuai kebutuhan project */}
             <Document file="/path-to-katalog-aksesoris.pdf" onLoadSuccess={onZipperDocumentLoadSuccess} loading={<div className="text-white">Memuat PDF...</div>}>
                <Page pageNumber={zipperPdfPageNumber} renderAnnotationLayer={false} renderTextLayer={false} className="shadow-2xl" />
             </Document>
          </div>
          {zipperNumPages && (
            <div className="mt-4 flex justify-center items-center gap-4 text-white">
              <button disabled={zipperPdfPageNumber <= 1} onClick={() => setZipperPdfPageNumber(p => p - 1)} className="px-3 py-1 bg-white/20 rounded disabled:opacity-50">Prev</button>
              <span>Halaman {zipperPdfPageNumber} dari {zipperNumPages}</span>
              <button disabled={zipperPdfPageNumber >= zipperNumPages} onClick={() => setZipperPdfPageNumber(p => p + 1)} className="px-3 py-1 bg-white/20 rounded disabled:opacity-50">Next</button>
            </div>
          )}
        </div>
      )}
